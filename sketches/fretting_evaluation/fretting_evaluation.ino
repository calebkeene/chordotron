...
#define fretter1OffPos 90
#define fretter1AOnPos 120
#define fretter1BOnPos 60
#define fretter2OffPos 60
#define fretter2AOnPos 30
#define fretter2BOnPos 100

// possible on position for the fretter on the other side OFF=110Â°, ON=130Â°
...
void setup() {
  ...
  noteOnHandler(0, startPitch, 50);
  noteOffHandler(0, startPitch, 50);
  delay(500);
  ...
  if (velocity > 0) {
    if (note == startingNote) {
      ...
      if (fretter1LastPlayedNearMiddle && fretter2LastPlayedNearMiddle) {
        ...
        moveStepper(fretterStepper2, abs(stepsToMove), HIGH, 505);
...
void loop() {
  ...
  // move between startFret and endFret 10 times
  // measure time taken
//  if (testFret) {
//    startTime = millis();
//    for (int i = 0; i < 10; i++) {
//      noteOnHandler(0, endPitch, 50);
//      noteOffHandler(0, endPitch, 50);
//
//      noteOnHandler(0, startPitch, 50);
//      noteOffHandler(0, startPitch, 50);
//    }
//    endTime = millis();
//    Serial.print("Time to fret from ");
//    Serial.print(startPitch);
//    Serial.print(" to ");
//    Serial.print(endPitch);
//    Serial.print(": ");
//    // fretter arm on fretter 1 takes 225 ms to apply and disengage the fretter arm
//    // fretter arm on fretter 2 takes 225 ms to apply and disengage the fretter arm
//    Serial.println((endTime - startTime) / 20 - 225);
//    // takes 225 ms to apply and disengage the frets
//    testFret = false;
//  }

  // drift
  // measure initial position of fret 1
  // play from C3# (49) to B4 (59) (ascending and descending)
  // B4 is fretted by the second fretter
  // measure new position of fret 1
  // initial position = 45.8 mm
  // final position after onve octave played ascending and descending =  mm
}
